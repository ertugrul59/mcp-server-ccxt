...
start all servers using the './start_servers_terminals.sh' script.
...
class MCPAgentStyleTester:
    ...
    async def initialize_mcp_client(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "📊 Using full MCP Config. Ensure all servers (Telegram, TradingView, Coinglass, CCXT) are running."
            )
            ...
            logger.error(
                "   HINT: This test requires ALL MCP servers to be running. Did you run './start_servers_terminals.sh'?"
            )
            return False
