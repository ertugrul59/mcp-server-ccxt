...
export function registerPublicTools(server: McpServer) {
  ...
  // 列出支持的交易所
  server.tool("list-exchanges", "List all available cryptocurrency exchanges", {}, async () => {
    return {
      content: [
        {
          type: "text",
  ...
  server.tool(
    ...
    async ({ exchange, symbol, marketType }) => {
      try {
        ...
        log(
          LogLevel.ERROR,
          `Error fetching ticker: ${error instanceof Error ? error.message : String(error)}`
        );
        return {
          content: [
            {
  ...
  server.tool(
    ...
    "Get ticker information for multiple trading pairs at once",
    {
      ...
      symbols: z
        .array(z.string())
        .describe("List of trading pair symbols (e.g., ['BTC/USDT', 'ETH/USDT'])"),
      marketType: z
        .enum(["spot", "future", "swap", "option", "margin"])
        .optional()
        .describe("Market type (default: spot)"),
      ...
      try {
        return await rateLimiter.execute(exchange, async () => {
          ...
          const tickers = await getCachedData(cacheKey, async () => {
            log(
              LogLevel.INFO,
              `Batch fetching tickers for ${symbols.length} symbols on ${exchange}`
            );
        ...
        log(
          LogLevel.ERROR,
          `Error batch fetching tickers: ${error instanceof Error ? error.message : String(error)}`
        );
        return {
          content: [
            {
  ...
  server.tool(
    ...
    async ({ exchange, symbol, limit }) => {
      try {
        ...
        log(
          LogLevel.ERROR,
          `Error fetching orderbook: ${error instanceof Error ? error.message : String(error)}`
        );
        return {
          content: [
            {
  ...
  server.tool(
    ...
    {
      exchange: z.string().describe("Exchange ID (e.g., binance, coinbase)"),
      symbol: z.string().describe("Trading pair symbol (e.g., BTC/USDT, BTC/USDT:USDT for perps)"),
    ...
    async ({ exchange, symbol, timeframe, limit, marketType }) => {
      try {
        return await rateLimiter.execute(exchange, async () => {
          const ex = marketType
            ? getExchangeWithMarketType(exchange, marketType)
            : getExchange(exchange);
          const cacheKey = `ohlcv:${exchange}:${
            marketType || "spot"
          ...
          const ohlcv = await getCachedData(cacheKey, async () => {
            log(
              LogLevel.INFO,
              `Fetching OHLCV for ${symbol} on ${exchange}, timeframe: ${timeframe}, limit: ${limit}`
            );
        ...
        log(
          LogLevel.ERROR,
          `Error fetching OHLCV data: ${error instanceof Error ? error.message : String(error)}`
        );
        return {
          content: [
            {
  ...
  server.tool(
    ...
    async ({ exchange, symbol, limit }) => {
      try {
        ...
        log(
          LogLevel.ERROR,
          `Error fetching trades: ${error instanceof Error ? error.message : String(error)}`
        );
        return {
          content: [
            {
    ...
    async ({ exchange, page, pageSize }) => {
      try {
        return await rateLimiter.execute(exchange, async () => {
          ...
          const allMarkets = await getCachedData(
            cacheKey,
            async () => {
        ...
        log(
          LogLevel.ERROR,
          `Error fetching markets: ${error instanceof Error ? error.message : String(error)}`
        );
        return {
          content: [
            {
    ...
    async ({ exchange, marketType }) => {
      try {
        return await rateLimiter.execute(exchange, async () => {
          ...
          const info = await getCachedData(
            cacheKey,
            async () => {
  ...
  server.tool(
    ...
    async ({ exchange, symbol, marketType }) => {
      try {
        return await rateLimiter.execute(exchange, async () => {
          ...
          const tiers = await getCachedData(
            ...
            async () => {
              log(
                LogLevel.INFO,
                `Fetching leverage tiers for ${
                  symbol || "all symbols"
                } on ${exchange} (${marketType})`
              );
              if (symbol) {
        ...
        log(
          LogLevel.ERROR,
          `Error fetching leverage tiers: ${error instanceof Error ? error.message : String(error)}`
        );
        return {
          content: [
            {
  ...
  server.tool(
    ...
    "Get current funding rates for perpetual contracts",
    {
      ...
      marketType: z
        .enum(["future", "swap"])
        .default("swap")
        .describe("Market type (default: swap)"),
    },
    async ({ exchange, symbols, marketType }) => {
      try {
        return await rateLimiter.execute(exchange, async () => {
          // Get futures exchange
          const ex = getExchangeWithMarketType(exchange, marketType);
          const cacheKey = `funding_rates:${exchange}:${marketType}:${
          ...
          const rates = await getCachedData(
            ...
            async () => {
              log(
                LogLevel.INFO,
                `Fetching funding rates for ${
                  symbols ? symbols.length : "all"
                } symbols on ${exchange} (${marketType})`
              );
              if (symbols) {
        ...
        log(
          LogLevel.ERROR,
          `Error fetching funding rates: ${error instanceof Error ? error.message : String(error)}`
        );
        return {
          content: [
            {
  ...
  server.tool(
    ...
    async ({ exchange }) => {
      try {
        ...
        log(
          LogLevel.ERROR,
          `Error fetching market types: ${error instanceof Error ? error.message : String(error)}`
        );
        return {
          content: [
            {
